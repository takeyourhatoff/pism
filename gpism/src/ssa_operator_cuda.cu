...
__global__ void apply_residual_kernel(
    int mx, int my, int gw, int stride_u, int stride_v, int stride_nu_u,
    int stride_nu_v, int stride_beta_u, int stride_beta_v, int stride_out_u,
  ...
  if (has_bc && mask_u && mask_u[mask_u_idx] != 0) {
    ...
    ax_u = flux_x * inv_dx2 + flux_y * inv_dy2 + coupling +
           beta_u[idx(i, j, gw, stride_beta_u)] * u[c_u];
    r_u[idx(i, j, gw, stride_r_u)] =
        b_u[idx(i, j, gw, stride_b_u)] - ax_u;
  }
  if (out_u) {
    out_u[idx(i, j, gw, stride_out_u)] = ax_u;
  }

  double ax_v = v[c_v];
  if (has_bc && mask_v && mask_v[mask_v_idx] != 0) {
    r_v[idx(i, j, gw, stride_r_v)] = 0.0;
  } else {
    const int e = idx(i + 1, j, gw, stride_v);
    const int w = idx(i - 1, j, gw, stride_v);
    const int n = idx(i, j + 1, gw, stride_v);
    ...
    ax_v = flux_x * inv_dx2 + flux_y * inv_dy2 + coupling +
           beta_v[idx(i, j, gw, stride_beta_v)] * v[c_v];
    r_v[idx(i, j, gw, stride_r_v)] =
        b_v[idx(i, j, gw, stride_b_v)] - ax_v;
  }
  if (out_v) {
    out_v[idx(i, j, gw, stride_out_v)] = ax_v;
  }
}

__global__ void apply_residual_region_kernel(
    int mx, int my, int gw, int stride_u, int stride_v, int stride_nu_u,
    int stride_nu_v, int stride_beta_u, int stride_beta_v, int stride_out_u,
    int stride_out_v, int stride_b_u, int stride_b_v, int stride_r_u,
    int stride_r_v, const double* u, const double* v, const double* nu_u,
    ...
    const double* b_u, const double* b_v, double* out_u, double* out_v,
    double* r_u, double* r_v, double inv_dx2, double inv_dy2, double inv_2dx,
    double inv_2dy, const int* mask_u, const int* mask_v, int stride_mask_u,
    int stride_mask_v, int has_bc, int i_start, int i_end, int j_start,
    int j_end) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x + i_start;
  const int j = blockIdx.y * blockDim.y + threadIdx.y + j_start;
  if (i >= i_end || j >= j_end) {
    return;
  }
...
void ssa_apply_residual_cuda(
    ...
    double inv_2dy, int stride_mask_u, int stride_mask_v, const int* mask_u,
    const int* mask_v, int has_bc) {
  CudaEventTimer timer("ssa_apply_residual");
  dim3 block(16, 16);
  dim3 grid_dim((mx + block.x - 1) / block.x,
                (my + block.y - 1) / block.y);
  apply_residual_kernel<<<grid_dim, block>>>(
      mx, my, gw, stride_u, stride_v, stride_nu_u, stride_nu_v, stride_beta_u,
      stride_beta_v, stride_out_u, stride_out_v, stride_b_u, stride_b_v,
      stride_r_u, stride_r_v, u, v, nu_u, nu_v, beta_u, beta_v, b_u, b_v,
      out_u, out_v, r_u, r_v, inv_dx2, inv_dy2, inv_2dx, inv_2dy,
...
void ssa_apply_residual_region_cuda(
  ...
  dim3 grid_dim((i_end - i_start + block.x - 1) / block.x,
                (j_end - j_start + block.y - 1) / block.y);
  apply_residual_region_kernel<<<grid_dim, block>>>(
      mx, my, gw, stride_u, stride_v, stride_nu_u, stride_nu_v, stride_beta_u,
      stride_beta_v, stride_out_u, stride_out_v, stride_b_u, stride_b_v,
      stride_r_u, stride_r_v, u, v, nu_u, nu_v, beta_u, beta_v, b_u, b_v,
      out_u, out_v, r_u, r_v, inv_dx2, inv_dy2, inv_2dx, inv_2dy, mask_u,
      mask_v, stride_mask_u, stride_mask_v, has_bc, i_start, i_end, j_start,
      j_end);
}
