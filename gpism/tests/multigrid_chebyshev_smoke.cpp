...
int main() {
  ...
  for (int j = 0; j < my; ++j) {
    for (int i = 0; i < mx; ++i) {
      ...
      b(i, j, 1) = static_cast<double>(3 * i - j);
      x(i, j, 0) = 0.0;
      x(i, j, 1) = 0.0;
    }
  }

  gpism::chebyshev_jacobi_smooth(grid, nuH, beta, b, x, 1, 1.0, 1.0);

  for (int j = 0; j < my; ++j) {
    for (int i = 0; i < mx; ++i) {
      if (!nearly_equal(x(i, j, 0), 0.5 * b(i, j, 0)) ||
          !nearly_equal(x(i, j, 1), 0.5 * b(i, j, 1))) {
        std::cerr << "chebyshev update mismatch at " << i << "," << j << "\n";
        return 1;
      }
    }
  }

  std::cout << "multigrid_chebyshev_smoke passed\n";
  return 0;
}
